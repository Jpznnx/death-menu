...
local noclip = {["Label"] = "CAPS",["Value"] = 137} 
menucolor = { r = 96, g = 96, b = 96 }
corzinhadomeni = { r = 96, g = 96, b = 96 }
magneto_cor = {r = 244, g = 244, b = 244}
...
ShowInfo("Aperte DELETE Para Abrir")
print("> DEATH MENU <")
print("> Jpznnx'#0001 <")
...
locals     = {
    ...
    selected2 = "",
    current_tab = "Jogador",
    meupau_color = { r = 192, g = 192, b = 192 },
...
function spawnarveiculo()
    ...
    if veiculoo and IsModelValid(veiculoo) and IsModelAVehicle(veiculoo) then
        ...
        ShowInfo("~b~[~w~" .. veiculoo .. "~b~]  ~m~Nao Existe!", bypasszinhazz)
    end
end
...
function tabs() 
    if tab == "Self" then
        ...
        if Button("Reviver MQCU ~g~[!]",bypasszinhaaa,0.325,0.43) then
            Citizen.InvokeNative(0xEA23C49EAA83ACFB, GetEntityCoords(PlayerPedId()), GetEntityHeading(PlayerPedId()), bypasszinhaaa, bypasszinhazz)
        end
        ...
        if Button("Desalgemar/Algemar ~y~[!]",bypasszinhaaa,0.344,0.46) then
            vRP.toggleHandcuff()
        end
        ...
        if Button("Tazer Em Geral ~g~[!]",bypasszinhaaa,0.3356,0.49) then
        ...
        if Button("~m~Desinjetar :(", bypasszinhaaa,0.3278,0.52) then
        ...
        if Checkbox("Freecam ~m~ [!]", 0.63, 0.560, 0.560, freecamm) then
...
--if Checkbox("Carro ~m~R~g~G~b~B ~g~[!]",0.63,0.380, 0.380,coloridin) then
...
if Checkbox("Particula 2~m~ GERAL",0.60,0.560, 0.560,hfkwebieewdqfogos5) then
...
if Checkbox("Particula 3~m~ GERAL",0.60,0.590, 0.590,hfkwebieewdqfogos6) then
                    ...
                    while DoesCamExist(camera) do
                        ...
                        if not spectarplayerpkrl then
                            ...
                            if coords ~= vector3(0, 0, 0) then
                                ...
                                local abc =
                                    getWidth(
                                    "[~Death Menu ~w~| ~y~Freecam~w~]  ~p~" .. currentmode .. "  ~m~Fora do Alcance!",
                                    4,
                                    0.5
                                )
                                local rX, rY = GetActiveScreenResolution()
        ...
        if platano.closed == bypasszinhaaa then
            ...
            for veh in platano:enumerate_vehicles() do				
                if IsEntityDead(veh) then
                    i = i + 1
                    if platano:button(GetDisplayNameFromVehicleModel(GetEntityModel(veh)) .. " [~m~ Destruído ~w~]",25,i * 16,255,255,255) then
                        SetVehicleFixed(veh)
